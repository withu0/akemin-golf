@props(['name', 'label', 'value' => null, 'type' => 'text', 'required' => false, 'hint' => null])
...
<div class="space-y-2">
    ...
    @elseif ($type === 'file')
        <input id="{{ $name }}" name="{{ $name }}" type="file" accept="image/*" {{ $attributes }} class="admin-input file:mr-3 file:border-0 file:bg-[var(--color-sumi)] file:text-[var(--color-paper)] file:px-3 file:py-1 file:rounded">
    @else
